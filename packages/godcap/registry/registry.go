...
//go:embed deployment/metadata.json
var metadataJSON []byte

//go:embed deployment/current
...
func initRegistry() {
	registryOnce.Do(func() {
		...
		for key, meta := range metadataMap {
			...
			// Read PCCS deployment
			pccsPath := filepath.Join("deployment", "current", chainIDStr, "onchain_pccs.json")
			pccsData, err := deploymentFS.ReadFile(pccsPath)
			if err != nil {
				// Network might not have deployment yet, skip
				continue
			}

			// Read DCAP deployment
			dcapPath := filepath.Join("deployment", "current", chainIDStr, "dcap.json")
			dcapData, err := deploymentFS.ReadFile(dcapPath)
			if err != nil {
